...
public class Login extends javax.swing.JPanel {
    ...
    private void loginBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loginBtnActionPerformed
        ...
        if (username.length() < 4 || password.length() < 8) {
            javax.swing.JOptionPane.showMessageDialog(this, "Invalid credentials. Ensure username is ≥ 4 and password is ≥ 8 characters.");
            return;
        }
        ...
        for (Model.User user : users) {
            if (user.getUsername().equals(username)) {
                if (user.getLocked() == 1) {
                    javax.swing.JOptionPane.showMessageDialog(this, "This account is locked.");
                    return;
                }
                if (Utils.SecurityUtil.verifyPassword(password, user.getPassword())) {
                    frame.main.currentUser = user; // Save logged-in user
