...
public class Frame extends javax.swing.JFrame {
    ...
    private void logoutBtnActionPerformed(java.awt.event.ActionEvent evt) {
        if (main.currentUser != null) {
            main.sqlite.addLogs("LOGOUT", main.currentUser.getUsername(), "User logged out", java.time.LocalDateTime.now().toString());
        }
    ...
    public CardLayout contentView = new CardLayout();
...
public boolean loginAction(String username, String password) {
    for (Model.User user : main.sqlite.getUsers()) {
        if (user.getUsername().equals(username)) {

            if (user.getLocked() == 1) {
                javax.swing.JOptionPane.showMessageDialog(this, "Account is locked. Please contact admin.");
                main.sqlite.addLogs("FAILED LOGIN", username, "Account is locked", java.time.LocalDateTime.now().toString());
                return false;
            }

            if (Utils.SecurityUtil.verifyPassword(password, user.getPassword())) {
                main.currentUser = user;
                main.sqlite.addLogs("LOGIN", username, "Login successful", java.time.LocalDateTime.now().toString());
                ...
                switch (user.getRole()) {
                    ...
                    case 2:
                        ...
                        break;
                    default:
                        javax.swing.JOptionPane.showMessageDialog(this, "Unknown role.");
                        return false;
                }
                ...
                main.sqlite.addLogs("FAILED LOGIN", username, "Incorrect password", java.time.LocalDateTime.now().toString());
                return false;
            }
        }
    }
    ...
    main.sqlite.addLogs("FAILED LOGIN", username, "Username not found", java.time.LocalDateTime.now().toString());
    return false;
}
